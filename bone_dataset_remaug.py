...
def gdist_appro(mat):
    mat = csr_matrix(mat)
    gdist, predecessors = shortest_path(csgraph= mat, method='D', directed= False, indices=None, return_predecessors=True) 
            ...
            if (predecessors[i][j] != -9999):
                ...
                if(node_num == 0):
                    ...
                    for k in range(node_num):
                        egr = egr + (edist[node_list[k]][node_list[k+2]] / gdist[node_list[k]][node_list[k+2]])
    ...
    for i in range(num):
        for j in range(i+1, num):
            if(path_index[i][j] == 1):
                ...
                if (egr != 0):
                    node_list = path_dict[str(i)+'-'+str(j)][0]
                    node_hops = len(node_list) - 2     #remove the start point and the goal point
                    #egr = ave_egr[i][j]
                    if(egr > ratio):
                        g = gdist[i][j]
        ...
        for j in range(i+1, num):
            if(path_index[i][j] == 1):
                ...
                if(egr != 0):
                    ...
                    node_hops = len(node_list) - 4     #remove the start point and the goal point，and the successor
                    if(egr < ratio):
                        g = gdist[i][j]
                        if (node_hops < (int(g * unit_hop))):
                            seg_egr = []
                            ...
                            add_node = []
                            for k in range(node_hops):
                                seg_egr.append(ave_egr[node_list[k+1]][node_list[k+3]])
                                add_candi.append([node_list[k],node_list[k+4]])
                            ...
                            for n in range (min( node_hops, (int(g * unit_hop)) - node_hops)):
                                ...
                                add_tag[add_candi[index][0]][add_candi[index][1]] = add_tag[add_candi[index][0]][add_candi[index][1]] + 1
...
def interpolation_optimize(dataset, row, col, path_dict, edist, sam_num):
    ...
    while ( i <= sam_num and sample_num <= 10000):
        ...
        if ( (lam * mu) <= 0 and  (np.sqrt(sample.dot(sample)) <= radius) ):
            ...
            if( lam <= 0):
                avg_egr = (e_sn / (e_sm + e_mn) + e_mg / (e_ng + e_mn) + e_nx / (e_ng + e_gx)) / 3
            else:
                avg_egr = (e_sm / (e_sn + e_mn) + e_ng / (e_mg + e_mn) + e_pn / (e_ps + e_sn)) / 3
...
def dataset_augment(dataset, add_tag, percentage, edist, path_dict):
    ...
    for i in range(topk):
        ...
        if(add_tag[row][col] == 0):
            break
        new_data = interpolation_optimize(dataset, row, col, path_dict, edist, sam_num)
...
def hyper_computation( knn, label, share_dict):
    hyper_uhop = [ ]
    hyper_ratio = [ ]
    #dataset = dv.class_read('mnist', label)
    ...
    #print("dict, ave_egr")
    #print("label",label)
    share_dict[label] = [dataset, ave_egr, path_dict, edist, gdist, path_index, weight, hyper_ratio, hyper_uhop]
    #print("label",label)


def whole_remove(dataset,ave_egr, path_dict, gdist,path_index, weight, unit_hop, ratio, percentage):

    remove_tag = dataset_compression_index(ave_egr, path_dict, gdist, unit_hop, ratio, path_index, weight)
    print("remove_tag")
    #print(remove_tag)
...
def whole_augment(dataset, ave_egr, path_dict, gdist, edist, path_index, weight, unit_hop, ratio, percentage):
    ...
    add_tag = dataset_augment_index(ave_egr, path_dict, gdist, unit_hop, ratio, path_index, weight)
    #print("add_tag", add_tag)
    asub_data, add_data = dataset_augment(dataset, add_tag, percentage, edist, path_dict)
